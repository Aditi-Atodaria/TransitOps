...
<<<<<<< HEAD
  ...
  try {
    switch (state.tab) {
      ...
      case "drivers": await renderDrivers(main); break;
      case "trips": await renderTrips(main); break;
      case "maintenance": await renderMaintenance(main); break;
      case "fuel": await renderFuel(main); break;
...
function enhanceView(main) {
  ...
  // restart the entrance animation on each render
  main.style.animation = "none";
  void main.offsetWidth;
  main.style.animation = "";

  // stagger table row reveals
  main.querySelectorAll(".data-table tbody tr").forEach((tr, i) => {
    tr.style.animationDelay = Math.min(i * 35, 500) + "ms";
  });

  // stagger KPI tiles + count up their numeric values
...
function countUp(el) {
  ...
  function frame(now) {
    ...
    else el.textContent = target.toLocaleString("en-IN", { minimumFractionDigits: decimals, maximumFractionDigits: decimals }) + suffix;
  }
